...
function orderByYear(moviesArray) {
    let copiedArray = moviesArray.map((element) => {
        return element;
    });
...
function bestYearAvg(moviesArray) {
    ...
    for (i = 0; i < copiedArray.length - 1; i++) {
        if ((copiedArray[i].year != copiedArray[i + 1].year)) {
            endIndex = i + 1;
            yearlyArray.push(copiedArray.slice(startIndex, endIndex));
            startIndex = i + 1;
        }
    }
    yearlyArray.push(copiedArray.slice(startIndex));


    scoresArray = [];
    yearlyArray.forEach((element) => {
        scoresArray.push(scoresAverage(element));
    });


    maximumArray = findMax(scoresArray);
    indexOfMax = maximumArray[1];
    maxScore = maximumArray[0];
    bestYear = yearlyArray[indexOfMax][0].year;
